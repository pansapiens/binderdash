...
Each run will have a `project_id` and `run_name` that are intelligently guessed from the directory structure. The project ID guessing function will avoid disallowed names: 'runs', 'bindcraft', 'rfd', and numeric-only names, looking up the directory hierarchy to find an appropriate project identifier. The run name guessing function will avoid disallowed names using regex patterns: 'results.*' (e.g., 'results', 'results_1', 'results_final'), 'bindcraft', 'batches', and numeric-only names, looking up the directory hierarchy to find an appropriate name.
