...
def parse_designs_from_run(run_metadata: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Determine the design ID column and primary score column based on method
        if method == "bindcraft":
            ...
            # Unknown method, try to guess columns
            design_id_col = None
...
def refresh_designs_cache():
    ...
    try:
        ...
        # Sort by score, handling different methods
        def sort_key(design):
...
def get_default_plot_columns(df: pd.DataFrame, method: str) -> Dict[str, str]:
    """Get default column selections for plots based on method and available columns."""
...
async def get_plot_columns_multiple(
    ...
    try:
        ...
        # Get default columns based on the most common method
...
async def download_pdbs_tar(
    ...
    try:
        ...
        # Build list of actual file paths to include
        file_entries: List[tuple[str, Path]] = []
        for item in request.items:
            run = get_run_metadata(item.run_id)
            if not run:
                logger.warning(f"Run not found for tar request: {item.run_id}")
                continue
            ...
            if matched and matched.exists():
                # Use a relative name inside the tar: project/run_name/filename
        ...
        logger.error(f"Error creating PDBs tar: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
