...
- **Prepare sequences tab**: N/C tag presets (inline in `seqPrep.ts` as `TAG_PRESET_DEFS`) and custom peptides, terminal additions, optional stop, good-only scope, sequence extraction from structures (`POST /api/designs/sequences`), E. coli reverse translation, preview, and FASTA/TSV/CSV export; persists `Sequence` and `binder_chain` (ingest/`chain` normalisation; tag placement and tag-metrics may backfill when persistence is on). Chips use PrimeVue passthrough (`pt`) so theme styles do not override preset colours.
