...
- **Changed (Filtering / performance)**: **Compute target contacts is ~20x faster.** The Shrake-Rupley kernel is now biotite's rather than `Bio.PDB.SASA`, which built a fresh KDTree of the sphere mesh for every atom and did Python set arithmetic over point indices. BioPython's conventions are kept exactly - its `ATOMIC_RADII` and its golden-spiral point mesh are passed to biotite as parameters - so the stored numbers are unchanged and cached records stay valid. Two further savings: only residues inside the 12 A record cutoff get an area computed (everything else still occludes them), and the apo pass drops the binder from the occluder set instead of deep-copying the structure. Measured on the bundled ccl7 run (64 designs, moving target, so both SASA passes run): 648 -> 34 ms per design single-threaded, 195 -> 9 ms on four cores. Parity against the previous implementation is pinned by tests rather than by golden values, so the numbers stay comparable with nf-binder-design's `complex_sasa.py`.
