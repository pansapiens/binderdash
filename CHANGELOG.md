...
- **Added (Downloads)**: **Design bundles** - a single zip carrying the data *and* the state that produced it. The Designs tab's download button now defaults to **Download Design Bundle (zip)** (TSV, CSV, PDBs and FASTA remain in the dropdown), and Prepare Sequences defaults to a superset bundle. A bundle holds `designs.tsv`, `binders.fasta`, the structure files, cached target-contact SASA (`target_contacts.tsv` plus a per-run apo reference), and `binderdash_session.json` recording the selected runs, table sort, hard filters, target-contact conditions and ranking configuration. The Prepare Sequences bundle adds `constructs.tsv`, amino acid and nucleotide FASTAs, the vendor CSV, and `prepare_sequences.json` with the tags, stop/padding, short-name strategy and codon optimisation settings. Every JSON file is schema-versioned, carries the app version and git commit, and ships with its JSON Schema under `schemas/`; `manifest.json` inventories every member with a sha256. Saved Sets download the same bundle, and now record UI state when saved, so a set downloaded later reproduces the table layout and not just the filter recipe. New endpoints `POST /api/bundles/designs`, `/prepare-sequences` and `/estimate` (a pre-flight size check).
- **Added (UI)**: **Restore session from JSON** in the app header reads a `binderdash_session.json` or `prepare_sequences.json` back into the interface, reporting per section what was applied and which runs are no longer on the server.
- **Added (Sequence prep)**: Prepare Sequences settings - tags, stop codons, short-name strategy, codon table and optimisation constraints - now survive a page reload. Optimised DNA is deliberately not restored, since it is derived from those settings and stale sequences shown as current would be worse than none.
- **Changed (Saved sets)**: `GET /api/saved-sets/{id}/download` returns the standard bundle rather than its previous `designs.csv` + `structures/` zip. Structure files missing from disk, and two runs sharing a structure basename, were previously dropped silently; both are now reported in the manifest's warnings and coverage counts.
...
- **Added (Filtering / UI)**: **Target contacts** - a new "2. Target contacts" section in the Filtering tab applies structure-derived conditions on target residues as hard filters, alongside the ordinary column filters. Each condition names target residues (multi-select with search) and a comparison: within / further than X Å of the binder (CA, CB or heavy-atom distance), SASA when bound, or ΔSASA on binding, in Å² or percent of the residue's theoretical maximum (Tien et al. 2013). `scope` applies it to any / all / at least N of the listed residues, or to their summed total. Multiple target subsections let one filter set name equivalent residues under different targets' numbering; a design whose run falls outside a subsection's scope is exempt from it.
- **Added (Filtering / API)**: Per-design per-target-residue contacts are computed on demand (Compute button, batched) and cached in the database, keyed by structure, chain roles and computation parameters. Records are stored for residues within 12 Å of the binder - beyond that ΔSASA is 0 and bound SASA equals the free-target value, both recoverable from the per-run reference. New endpoints `POST /api/filtering/target-residues`, `/target-contacts/compute` and `/target-contacts/profile`; `target_contact_groups` is accepted by the preview, apply, rank, diversity and run endpoints. Targets are identified by sequence rather than chain ID, so one target spans runs that letter its chain differently.
- **Added (UI)**: **Target contact map** - a collapsible section under the Designs tab structure viewer colours the target by mean ΔSASA on binding, contact frequency, or closest approach, over the whole run or just the selected designs. ΔSASA is shown as a percentage of each residue's theoretical maximum by default, with the gradient topping out at 30% so its contrast lands at the epitope boundary rather than on the few residues that bury most of their area. The colour key is captioned with the scale in use (`% ΔSASA on binding`, `Closest approach (Å, heavy atoms)`, `Designs within 5 Å (CB)`), since the numbers either side of a gradient say nothing about what they measure. The key and its dual-handle range slider sit directly under the viewer (only while the map is on, so they cost nothing otherwise); the slider spans the observed range, and pulling the handles in changes the key's labels to `<= min` / `>= max`. With a contact / no-contact boolean mode and a palette choice, a design set spread over several epitopes can be tuned until the pattern shows rather than averaging into a blur.
- **Changed (Filtering / performance, SASA conventions)**: **Compute target contacts is ~24x faster**, and target-residue SASA now uses biotite throughout. `Bio.PDB.SASA` built a fresh KDTree of the sphere mesh for every atom and did Python set arithmetic over point indices; the kernel is now biotite's. Two further savings: only residues inside the 12 A record cutoff get an area computed (everything else still occludes them), and the apo pass drops the binder from the occluder set instead of deep-copying the structure. Measured on the bundled ccl7 run (64 designs, moving target, so both SASA passes run): 648 -> 27 ms per design single-threaded, 195 -> 8 ms on four cores.
- **Changed (Filtering / Sequence prep) - BREAKING for cached contacts**: SASA now uses **ProtOr radii (Tsai et al. 1999) over heavy atoms**, replacing BioPython's per-element `ATOMIC_RADII` with explicit hydrogens. ProtOr varies the radius by atom environment rather than element alone, and dropping modelled hydrogens stops the areas depending on whether the folding method wrote them. Absolute areas shift by a few percentage points per residue (mean ~0, max ~7 pp on the bundled PD-L1 run) and residue rankings are essentially unchanged, so the 30% defaults still land at the epitope boundary - 4-9 residues per design clear them. `params_key` now names the radii set, so records cached under the old convention are never read back; press **Compute target contacts** again to repopulate. `tag_placement`'s `%SASA` moved with it, since both features normalise against the same Tien 2013 maxima and would otherwise disagree about the numerator. Both now share `backend/util/sasa.py`.
- **Changed (Filtering / UI)**: The "target moves between designs" notice, previously one info box per affected run, is now a single line inside the coverage box, naming every affected run in one comma-separated list.
- **Changed (Filtering / UI)**: A new target contact condition now defaults to `ΔSASA on binding ≥ 30% of max` scored as a site total, matching the contact map's own defaults, rather than `within 5 Å of binder` scored per-residue.
- **Fixed (Filtering / UI)**: A target contacts section restored from a previous session kept the target key of whatever run was selected then, so changing the run selection left the residue dropdown empty and the section silently matching nothing. Sections with no residues chosen now follow the scope; sections that do carry residue selections are left alone but say the target is out of scope and offer the target picker (previously hidden whenever the scope held a single target) so they can be re-pointed or removed.
- **Added (MCP)**: `query_designs`, `rank_designs` and `select_diverse_designs` accept `target_contact_groups`; new tools `list_target_residues` and `target_contact_profile`. Computing contacts is deliberately not a tool - it parses every structure - so uncomputed runs produce a `TARGET_CONTACTS_UNAVAILABLE` warning and an explicit empty-result explanation rather than a confidently empty answer.
...
- **Added (Auth / API)**: **`PAM_GECOS_EMAIL`** — optionally resolve a PAM user's email from the GECOS **5th field only** ("other"); default `false`. Restricted to that field because `chfn` cannot write it (unlike fields 1–4 under the usual `CHFN_RESTRICT="rwh"`), so trusting an earlier field would let a user claim a colleague's email and be merged into their account.
- **Fixed (Auth / Docker)**: PAM logins failed in Docker even with host `/etc/passwd`, `/etc/group` and `/etc/shadow` bind-mounted. `pam_unix` shells out to the setgid-`shadow` helper `/sbin/unix_chkpwd`, which will not check a user other than its caller, so a container running as a non-root `user:` always got `PAM_AUTHINFO_UNAVAIL` — reported as an indistinguishable "Incorrect username or password". The compose services now set `group_add: ["42"]`. Documented alongside it: those single-file mounts pin an inode, so host password changes and new users need `docker compose up -d --force-recreate`, not `restart`.
- **Changed (Auth / API)**: `GET /api/auth/status` no longer reports `providers.api_key`; it now returns a top-level `api_keys: {enabled: bool, reason: string|null}` (`reason` is `null`, `"auth_disabled"`, or `"persistence_disabled"`).
- **Removed (Auth / API) — BREAKING**: The single shared `BINDERDASH_API_KEY` environment variable no longer exists; `settings.api_key_enabled()` is gone. Scripted access now requires a per-user key (see above). Existing deployments relying on the shared key must mint per-user keys before upgrading.
- **Added (MCP)**: **MCP server at `/api/mcp/`** — stateless streamable-HTTP, mounted in the same FastAPI process (so it shares the design cache), authenticated with the existing per-user `bd_` API keys via `Authorization: Bearer` or `X-Binderdash-Api-Key`. Twelve tools: `list_runs`, `describe_methods`, `describe_columns`, `query_designs`, `summarize_designs`, `rank_designs`, `select_diverse_designs`, `saved_sets`, `extract_sequences`, `inspect_structures`, `read_structure_file`, `export_structures`. Read + analysis + Saved Sets only — no scan/ingest, deletion, cache refresh or merge-table upload. **Enabled by default in Docker** — `backend/Dockerfile` compiles with `--extra mcp`, so both compose stacks serve it (an existing stack needs `docker compose up -d --build` once). Local venvs opt in with `uv pip install -e "backend[mcp]"`; the desktop build deliberately excludes it. Without `fastmcp` the endpoint is simply not mounted and nothing else changes. See [`docs/development/mcp.md`](docs/development/mcp.md).
- **Fixed (UI)**: The API key expiry dropdown showed a blank box after choosing **Never**. That option carried `value: null`, and PrimeVue's `Select` gates label rendering on `$filled` (`isNotEmpty(modelValue)`), so a null selection is indistinguishable from no selection and falls back to the placeholder — of which there is none. "Never" is now a `0` sentinel, mapped back to `null` when the key is created.
- **Added (MCP / UI)**: The **New API key created** panel now offers one-click AI agent setup — a `claude mcp add` command and an equivalent `mcpServers` JSON snippet, both pre-filled with the new key and this server's URL, plus where to paste them (`~/.claude.json` or a project `.mcp.json`). Shown only when the server has MCP mounted: `GET /api/auth/status` gained `mcp: {enabled, path}`.
- **Changed (MCP)**: The tool surface deliberately does not mirror REST — canonical metric names resolve per method, sort directions are applied from a server-side vocabulary rather than guessed, server filesystem paths are never returned (`structure_filename`/`structure_url` instead of `pdb_file`), sort/limit/offset/column-selection happen server-side, unresolvable ranking metrics are errors rather than silently skipped terms, and oversized responses are refused rather than truncated.
- **Fixed (Filtering) — BREAKING behaviour**: Diversity selection no longer blank-fills missing sequences. A blank sequence aligns to zero against every other design, so its normalised identity was `0` — maximally *dissimilar* — and the diversity term therefore **preferred** designs with no sequence, quietly seeding curated panels with them. Sequenceless designs are now excluded from the diversity pool, so a diverse set can legitimately be smaller than the requested budget.
- **Added (Filtering)**: `POST /api/filtering/diversity` and `POST /api/filtering/run` return a `warnings: string[]` explaining a diverse set that is empty or short of the budget (no `Sequence` column at all, or *N of M* passing designs lacking sequences). Previously this surfaced only as an unexplained `diverse_set_count: 0`. The UI reports these as a warning toast rather than an unqualified success.
- **Fixed (Filtering)**: Comparing two empty sequences no longer divides by zero (`max(0, 0)`); identical empty sequences score as identical, not maximally diverse.
- **Removed**: The unused `prototypes/` folder.
