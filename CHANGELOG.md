...
- **SQLite database**: **`DATABASE`** (SQLite by default) stores ingested runs and per-design **tag**/**good** (no CSV writes). **`POST /api/runs/scan`** (disk only; optional **`force_rescan_of_ingested`**), **`/ingest`**, **`/ingest-preview`**; stable **`run_id`** per **`run_group_key`**. Tabs: **Designs**, **Select Runs**, **Plots**, **Ingest Runs**; Designs use **Select Runs** selection only. Re-ingest is confirmed by name (resets tag/good).
- **Tag placement (Designs)**: Collapsible batch His-tag prediction/metrics (**`POST /api/designs/tag-placement`**, **`tag-metrics`**); Mol\* toolbar **N/C** toggle and **`PATCH /api/designs/tag`** (DB). Options saved in **`localStorage`** per run.
