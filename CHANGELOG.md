...
- **Mol\* reference toggle**: The **Ref** visibility choice persists when changing design or run; reference reload no longer clears the blob URL before the new fetch (which had reset visibility). Clearing the overlay from Advanced options resets preference to visible for the next load. PDBTM membrane plane overlay hides and shows with the reference.
